...
class H:
    ...
    def __init__(mysillyobject, cust, sum_1_quant, count_1_quant, avg_1_quant):
...
def query():
    ...
    for row in cur:
        ...
        for y in ['sum_1_quant', 'count_1_quant', 'avg_1_quant']:
            hInstan[y] = None
        ...
        for row in cur:
            ...
            for i in split_key:
                if row[i[0]] != i[1]:
                    isUsed = False
            if isUsed:
                if not(eval("row['state']=='NJ' or row['month'] > 3")):
                    isUsed = False
                if isUsed:
                    agg_instance.append(row)  
        for x in ['sum_1_quant', 'count_1_quant', 'avg_1_quant']: # for calculating the aggregate functions for the H-class table
            split_x = x.split("_")
